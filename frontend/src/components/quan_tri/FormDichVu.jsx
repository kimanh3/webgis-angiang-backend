// src/components/quan_tri/FormDichVu.jsx
...
import {
  ...
  updateDichVuChung,
  deleteDichVuChung,
} from "../../api/dichvuchungApi";

export default function FormDichVu() {
  ...
  const loadList = async () => {
    ...
    try {
      setMessage(null);
      const fc = await listDichVuChung();
      ...
      setItems(
        features.map((f) => {
          ...
          return {
            ...
            loai_dv: pick(p, ["loai_dv", "loai_dich_vu", "type"], ""),
          };
        })
      );
    } catch (err) {
      console.error(err);
      setMessage(err?.message || "❌ Lỗi tải danh sách dịch vụ chung.");
    } finally {
  ...
  // ✅ options loại dịch vụ (tự lấy từ dữ liệu)
  const typeOptions = useMemo(() => {
    const set = new Set();
    items.forEach((it) => {
      const v = (it?.loai_dv || "").trim();
      if (v) set.add(v);
    });
    return Array.from(set).sort((a, b) => a.localeCompare(b, "vi"));
  }, [items]);

  // ✅ lọc keyword + loại dịch vụ
  const filteredItems = useMemo(() => {
    const q = (keyword || "").trim().toLowerCase();
    const t = (typeFilter || "ALL").trim();

    return items.filter((it) => {
      const matchType = t === "ALL" ? true : (it?.loai_dv || "").trim() === t;
      if (!matchType) return false;

      if (!q) return true;

      const s = `${it.gid ?? ""} ${it.ten ?? ""} ${it.dia_chi ?? ""} ${it.loai_dv ?? ""}`.toLowerCase();
      return s.includes(q);
    });
  ...
  const handleEdit = async (gid) => {
    ...
    try {
      setMessage(null);
      const data = await getDichVuChungById(gid);

      setEditingGid(gid);
      setForm({
        ten: pick(data, ["ten", "name"], ""),
        dia_chi: pick(data, ["dia_chi", "diachi", "dia_chi_1"], ""),
        loai_dv: pick(data, ["loai_dv", "loai_dich_vu", "type"], ""),
        gio: pick(data, ["gio", "gio_hoat_dong", "gio_mo_cua"], ""),
        sdt: pick(data, ["sdt", "so_dien_thoai", "phone"], ""),
        lat: pick(data, ["lat", "y"], ""),
        lng: pick(data, ["lng", "long", "x"], ""),
      });
    } catch (err) {
      console.error(err);
      setMessage(err?.message || "❌ Lỗi lấy chi tiết điểm dịch vụ chung.");
    } finally {
  ...
  const handleDelete = async (gid) => {
    if (!window.confirm("Xoá điểm dịch vụ chung này?")) return;
    try {
      setMessage(null);
      await deleteDichVuChung(gid);
      setMessage("✅ Đã xoá điểm dịch vụ chung.");
      if (editingGid === gid) handleCancelEdit();
      loadList();
    } catch (err) {
      console.error(err);
      setMessage(err?.message || "❌ Lỗi xoá điểm dịch vụ chung.");
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingGid) {
        await updateDichVuChung(editingGid, payload);
        setMessage("✅ Đã cập nhật điểm dịch vụ chung.");
      } else {
        await createDichVuChung(payload);
        setMessage("✅ Đã tạo điểm dịch vụ chung mới.");
      }
  ...
  return (
    <div style={ui.wrap}>
      <div style={ui.titleRow}>
        <h3 style={ui.h3}>Quản lý điểm dịch vụ chung</h3>
      ...
      <div style={isMobile ? { ...ui.layout, gridTemplateColumns: "1fr" } : ui.layout}>
        ...
        <div style={ui.card}>
          <div style={ui.cardHeader}>
            <div style={ui.cardTitle}>Danh sách dịch vụ chung</div>
          ...
          <div style={ui.toolbar}>
            <div style={ui.searchGroup}>
              <input
                ...
                placeholder="Tìm theo tên / địa chỉ / loại dịch vụ..."
              />
            ...
            <table style={ui.table}>
              <thead>
                <tr>
                  ...
                  <th style={{ ...ui.th, ...ui.colLoai }}>Loại dịch vụ</th>
                    ...
                    return (
                      <tr key={gidKey}>
                        ...
                        <td style={{ ...ui.td, ...ui.colLoai, background: bg }}>
                          <div style={ui.clamp2} title={it.loai_dv || ""}>
                            {it.loai_dv || "-"}
                          </div>
                        </td>
        ...
        <form style={ui.card} onSubmit={handleSubmit}>
          <div style={ui.cardHeader}>
            <div style={ui.cardTitle}>
              {editingGid ? `Sửa điểm dịch vụ chung (gid: ${editingGid})` : "Thêm điểm dịch vụ chung mới"}
            </div>
          ...
          <div style={ui.formBody}>
            <label style={ui.label}>
              ...
              <input
                ...
                required
                placeholder="VD: Bến xe / Trạm / Dịch vụ..."
              />
            </label>
            ...
            <label style={ui.label}>
              ...
              <input
                style={ui.input}
                value={form.loai_dv}
                onChange={(e) => handleChange("loai_dv", e.target.value)}
                placeholder="VD: Vận chuyển / Y tế / ATM..."
              />
            </label>

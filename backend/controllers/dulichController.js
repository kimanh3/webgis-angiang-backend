...
exports.getDiaDiemDuLichAG = async (_req, res) => {
  try {
    ...
    const { rows } = await pool.query(fcSql("public.diadiemdulich", props));

    res.set("Content-Type", "application/json");
    res.status(200).json(rows?.[0]?.geojson ?? { type: "FeatureCollection", features: [] });
  } catch (err) {
    console.error("❌ Lỗi lấy dữ liệu du lịch:", err);
    res.status(500).json({ error: "Không thể lấy dữ liệu điểm du lịch An Giang" });
  }
};
...
exports.getDiemDuLichById = async (req, res) => {
  try {
    ...
    const sql = `
      SELECT
        gid, id, ten, dia_chi, loai_hinh, gio_mo_cua, mo_ta, hinh_anh,
        ten_xa,
        ST_Y(geom) AS lat,
        ST_X(geom) AS lng,
 ...
 * Body: { ten, dia_chi, loai_hinh, gio_mo_cua, mo_ta, hinh_anh, ten_xa, lat, lng }
 */
exports.createDiemDuLich = async (req, res) => {
  try {
    const { ten, dia_chi, loai_hinh, gio_mo_cua, mo_ta, hinh_anh, ten_xa, lat, lng } = req.body;
 ...
 * Cập nhật điểm du lịch (✅ FIX TRIỆT ĐỂ lỗi kiểu lat/lng)
 */
exports.updateDiemDuLich = async (req, res) => {
  try {
    ...
    // ép về string để NULLIF($8::text,'') luôn hoạt động ổn
